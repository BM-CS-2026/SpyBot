...
const Render = {
  ...
  _bullets(arr) {
    ...
    return `<ul class="bullet-list">${arr.map(b => `<li>${escHtml(b)}</li>`).join('')}</ul>`;
  },
  ...
  _sectionProfessional(data) {
    ...
    const patBody = p.patents && p.patents.length
      ? `<ul class="bullet-list">${p.patents.map(pt => `<li><strong>${escHtml(pt.title || '')}</strong>${pt.number ? ` — ${escHtml(pt.number)}` : ''}${pt.year ? ` (${escHtml(pt.year)})` : ''}</li>`).join('')}</ul>`
      : '<div class="notfound">Not found.</div>';

    const pubBody = p.publications && p.publications.length
      ? `<ul class="bullet-list">${p.publications.map(pub => `<li><strong>${escHtml(pub.title || '')}</strong>${pub.venue ? ` — ${escHtml(pub.venue)}` : ''}${pub.year ? ` (${escHtml(pub.year)})` : ''}</li>`).join('')}</ul>`
      : '<div class="notfound">Not found.</div>';
    ...
    const socBody = p.social && p.social.length
      ? `<ul class="bullet-list">${p.social.map(s => `<li><strong>${escHtml(s.platform || '')}:</strong> ${s.url ? `<a href="${escAttr(s.url)}" target="_blank" rel="noopener" style="color:var(--cyan-soft)">${escHtml(s.url)}</a>` : ''}${s.themes ? ` — ${escHtml(s.themes)}` : ''}</li>`).join('')}</ul>`
      : '<div class="notfound">Not found.</div>';
    ...
    const inner = `
      <div class="subgroup">
        <div class="subgroup-title">Education</div>${eduBody}
      </div>
      <div class="subgroup">
        <div class="subgroup-title">Patents</div>${patBody}
      </div>
      <div class="subgroup">
        <div class="subgroup-title">Publications</div>${pubBody}
      </div>
      <div class="subgroup">
        <div class="subgroup-title">Awards & Recognition</div>${awardsBody}
      </div>
      <div class="subgroup">
        <div class="subgroup-title">Social</div>${socBody}
      </div>
      <div class="subgroup">
        <div class="subgroup-title">Recent news</div>${newsBody}
      </div>
    `;
